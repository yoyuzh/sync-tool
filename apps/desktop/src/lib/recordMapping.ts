import type { ClipboardRecord, PublishState } from "@sync-tool/shared";
import type { HistoryFilter, RecordStatus, StatusTone, UiRecord } from "../types/ui";

export function toUiRecord(record: ClipboardRecord): UiRecord {
...
function toRecordStatus(publishState: PublishState): RecordStatus {
...
const STATUS_META: Record<RecordStatus, { label: string; tone: StatusTone }> = {
...
function buildFilterTags(record: ClipboardRecord, status: RecordStatus): HistoryFilter[] {
  const tags: HistoryFilter[] = ["all"];
...
function formatSize(sizeBytes: number): string | undefined {
