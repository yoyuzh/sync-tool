...
import { emitConnectionChanged, emitHistoryChanged, emitNotification, registerIpc } from "./ipc/registerIpc";
...
app.whenReady().then(async () => {
    ...
    const historyStore = new LocalHistoryStore(path.join(userDataPath, "history.json"), initialSettings.maxLocalHistoryItems);
    ...
    async function publishCurrentClipboard() {
        ...
        try {
            ...
            notificationService.showStatus("发送失败", error instanceof Error ? error.message : "服务端暂不可用，记录已保留在本地");
        }
    }
    async function pasteLatestOnlineRecord() {
        try {
            const records = await historyStore.list();
            const record = records.find((item) => (item.publishState === "published" || item.publishState === "broadcast") && item.textContent);
            if (!record) {
            ...
            notificationService.showStatus("粘贴失败", error instanceof Error ? error.message : "无法读取最近线上记录");
        }
    }
