...
export function registerIpc(options) {
    ...
    ipcMain.handle(IPC_CHANNELS.historyGet, async (_event, recordId) => {
        if (!isRecordId(recordId)) {
            return null;
        }
        return options.historyStore.get(recordId);
    });
    ipcMain.handle(IPC_CHANNELS.historyCopy, async (_event, recordId) => {
        if (!isRecordId(recordId)) {
    ...
    ipcMain.handle(IPC_CHANNELS.historyPublish, async (_event, recordId) => {
        if (!isRecordId(recordId)) {
            throw new Error("无效的记录 ID");
        }
        const record = await options.historyStore.get(recordId);
    ...
    ipcMain.handle(IPC_CHANNELS.clipboardWriteText, (_event, text) => {
...
export function emitConnectionChanged(win, status) {
    win?.webContents.send(IPC_CHANNELS.eventConnectionChanged, status);
}
export function emitNotification(win, item) {
...
function sanitizeSettingsPatch(value) {
    ...
    return compactSettingsPatch({
        ...
        clipboardPollingEnabled: typeof patch.clipboardPollingEnabled === "boolean" ? patch.clipboardPollingEnabled : undefined,
        clipboardPollingIntervalMs: typeof patch.clipboardPollingIntervalMs === "number" ? patch.clipboardPollingIntervalMs : undefined,
        autoPublishEnabled: false,
        globalShortcutOpen: typeof patch.globalShortcutOpen === "string" ? patch.globalShortcutOpen : undefined,
        globalShortcutPublish: typeof patch.globalShortcutPublish === "string" ? patch.globalShortcutPublish : undefined,
        globalShortcutPasteLatestOnline: typeof patch.globalShortcutPasteLatestOnline === "string"
            ? patch.globalShortcutPasteLatestOnline
            : undefined,
        notificationPreviewEnabled: typeof patch.notificationPreviewEnabled === "boolean" ? patch.notificationPreviewEnabled : undefined,
        maxLocalHistoryItems: typeof patch.maxLocalHistoryItems === "number" ? patch.maxLocalHistoryItems : undefined
    });
}
function compactSettingsPatch(patch) {
