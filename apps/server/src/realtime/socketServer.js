...
export async function registerSocketServer(app) {
    app.get("/ws", { websocket: true }, (socket, request) => {
        ...
        const device = token
            ...
            : undefined;
        if (!device) {
            socket.send(JSON.stringify(makeServerMessage(SYNC_MESSAGE_TYPES.serverError, {
                code: "unauthorized",
        ...
        app.services.sessions.send(sessionId, makeServerMessage(SYNC_MESSAGE_TYPES.serverHello, {
            device: onlineDevice,
            protocolVersion: 1,
            serverTime: new Date().toISOString()
        }));
        app.services.sessions.send(sessionId, makeServerMessage(SYNC_MESSAGE_TYPES.presenceSnapshot, {
            devices: app.services.devices.listSessions(app.services.sessions.onlineDeviceIds())
        }));
        app.services.sessions.broadcast(makeServerMessage(SYNC_MESSAGE_TYPES.presenceChanged, {
            device: onlineDevice,
            online: true
        }), sessionId);
