...
export const recordsRoutes = async (app) => {
    app.post("/api/v1/records/publish", {
        schema: {
            body: {
                type: "object",
        ...
        app.log.info({ recordId: response.record.id, sourceDeviceId: device.deviceId }, "record published");
        return response;
    });
    app.get("/api/v1/records/:recordId", {
        schema: {
            params: {
                type: "object",
                required: ["recordId"],
