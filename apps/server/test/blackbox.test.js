...
describe("server black-box API and WebSocket behavior", () => {
    ...
    it("publishes a text record durably, lists it in retained history, and enforces idempotency conflicts", async () => {
        ...
        const conflicting = await publishRecord(baseUrl, registration.token, { ...draft, title: "Different" }, "request-1");
    ...
    it("returns stable error shapes for validation failures, missing records, and blob limits", async () => {
        ...
        const mismatchedPublish = await publishRecord(baseUrl, registration.token, mismatchedDraft, "mismatch-request");
        ...
        const invalidSourceFile = await publishRecord(baseUrl, registration.token, sourceFileWithText, "bad-source-file-request");
    ...
    it("paginates retained history and evicts records outside retention", async () => {
        ...
        const secondPage = await fetch(`${baseUrl}/api/v1/history?days=1&limit=10&cursor=${encodeURIComponent(firstPageBody.nextCursor)}`, { headers: authHeaders(registration.token) });
        expect(secondPage.status).toBe(200);
        const secondPageBody = await secondPage.json();
        expect(secondPageBody.records.map((record) => record.id)).not.toContain("old-record");
    });
    ...
    it("authenticates WebSocket sessions, sends presence messages, handles ping, and broadcasts published records", async () => {
        ...
        const socket = new WebSocket(`${baseUrl.replace("http://", "ws://")}/ws?token=${encodeURIComponent(listener.token)}`);
...
async function registerDevice(baseUrl, deviceName) {
...
function makeTextDraft(sourceDeviceId, id = "record-1") {
    return {
        id,
...
async function publishRecord(baseUrl, token, record, clientRequestId) {
...
function uploadBlob(baseUrl, token, recordId, body) {
...
async function expectDeviceOnline(baseUrl, token, deviceId) {
    for (let attempt = 0; attempt < 10; attempt += 1) {
        ...
        if (body.devices.some((device) => device.deviceId === deviceId && device.online)) {
            return;
        }
...
function collectUntil(socket, type) {
    return new Promise((resolve, reject) => {
